...
// COLD whole-warp-tile emission, outlined (__noinline__): serves unstaged tiles (misaligned
...
#  endif
     const int warp_tile_run_count = __reduce_add_sync(full_mask, __popc(my_word));
     if (lane_id == 0) { wt_counts[wt] = warp_tile_run_count; } __syncthreads();
     const auto [lane_warp_tile_run_count, lane_runs_before_warp_tile] =
       scan_warp_tile_run_counts<compute_warps>(wt_counts, lane_id);
     const int runs_before_warp_tile = __shfl_sync(full_mask, lane_runs_before_warp_tile, wt);
     const int tile_total_runs =
     ...
     if (wt == 0) {
       ...
       while (t > 0) {
         ...
         if (last_runs_tile < 0 && runs_mask != 0u) { last_runs_tile = base + (31 - __clz(runs_mask)); }
         if (cut >= 0) {
         ...
         if (tile_id == num_tiles - 1) { *d_num_runs = (NumRunsT) (prefix + tile_total_runs); }
       }
     } __syncthreads();
...
#  endif
     ...
     // unique keys straight from staged smem (v1 gather: one pass over rows, each lane stores
     // its own heads; the pass-1 drain bands are the receipted upgrade if this shows up)
     {
       const int my_popc = __popc(my_word);
       ...
       for (int iter = 0; iter < items_per_thread; ++iter) {
         const unsigned w = __shfl_sync(full_mask, my_word, iter);
         if ((w >> lane_id) & 1u) {
           const int rank = __shfl_sync(full_mask, my_runs_before_word, iter) + __popc(w & below);
#  ifndef RBK_FUSED_NOGATHER
           d_unique[global_runs_before_warp_tile + rank] =
