...
#  ifdef RBK_FUSED_RAWTREE
  // bug-hunt substitution: identical ascending count-predicated tree, raw intrinsics only
#    pragma unroll
  for (int offset = 1; offset < 32; offset <<= 1)
  {
    const ValueT r = shfl_down_sync_wide(acc, offset);
    if (lane_id + offset < n_valid)
    {
      acc = op(acc, r);
    }
  }
  return shfl_sync_wide(acc, 0);
#  else
