...
#  endif
  ...
  // CLC WORK-STEALING LOOP (this is a sparse problem: tile costs vary wildly within one launch,
  // so static assignment inherits the imbalance -- blocks steal canceled CTAs like pass 1's
  // loader). One block launch amortizes many tiles, and the NEXT stolen tile's TMA + metadata
  // prefetch issue at the steal point, a whole tile of lead time before their consumer.
  extern __shared__ char v_smem_raw[];
  ValueT* const staged_vals = (ValueT*) v_smem_raw;
  __shared__ ::cuda::std::uint64_t staged_bar;
  __shared__ __align__(16) uint4 v_clc_resp;
  __shared__ ::cuda::std::uint64_t v_clc_bar;
...
#  endif
  if (threadIdx.x == 0)
  {
    ...
    ptx::mbarrier_init(&v_clc_bar, 1);
    ptx::fence_proxy_async(ptx::space_shared);
    ptx::mbarrier_arrive_expect_tx(ptx::sem_release, ptx::scope_cta, ptx::space_shared, &v_clc_bar, 16);
    ptx::clusterlaunchcontrol_try_cancel(&v_clc_resp, &v_clc_bar);
    if (vals_staged && stage_run_threshold == 0)
    {
      // the first tile's TMA; every later tile's is issued a whole tile early at the steal point
  ...
  __syncthreads(); // barrier inits visible to every warp past this point
  int stage_it = 0;
  int n_issued = 0; // routed-mode stage count (uniform; the wait parity tracks issues, not tiles)
  for (int tile_id = (int) blockIdx.x; tile_id < num_tiles; ++stage_it)
  {
    ...
    if (vals_staged && tile_total_runs >= stage_run_threshold)
    {
      ...
      // always-stage: exactly one TMA per iteration (issued a tile early). Routed: counted issues.
    ...
    if (threadIdx.x < 32)
    {
      const int nxt = clc_next_tile_id(v_clc_resp, v_clc_bar, stage_it, num_tiles, (int) (threadIdx.x & 31));
      if (threadIdx.x == 0)
      {
        s_next_tile = nxt;
        if (nxt < num_tiles)
        {
          if (vals_staged && stage_run_threshold == 0)
          {
            const int nlen        = (int) min((OffT) tile_size, num_items - (OffT) nxt * tile_size);
            ...
            ptx::cp_async_bulk_ignore_oob(
              ...
              ptx::space_global,
              staged_vals,
              d_values + (size_t) nxt * tile_size,
              vspan,
              0u,
              vspan - vbytes,
              &staged_bar);
          }
          const char* nfbase = (const char*) (d_flag_words + (size_t) nxt * (compute_warps * 32));
          asm volatile("cp.async.bulk.prefetch.L2.global [%0], %1;" ::"l"(nfbase),
                       "r"((unsigned) (compute_warps * 32 * sizeof(unsigned)))
                       : "memory");
          const char* npbase = (const char*) ((size_t) (d_tile_prefix + nxt) & ~(size_t) 127);
...
#  pragma unroll
    ...
    __syncthreads(); // s_next_tile visible to every warp
    tile_id = s_next_tile;
  } // CLC work-stealing tile loop
}
