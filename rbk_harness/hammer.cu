...
int main(int argc, char** argv)
{
  ...
  using config_t = rbk_impl::winner_config<K, V, K_IPT, K_STAGES>;
  ...
  std::vector<V> hv((size_t) n); std::mt19937 vr(2); std::uniform_real_distribution<float> vd(0.f,1.f);
  for (auto& x : hv) x = vd(vr);
  std::vector<K> ru; std::vector<V> ra; // cpu ref
  for (long long j = 0; j < n; ++j) { if (j == 0 || h[j] != h[j-1]) { ru.push_back(h[j]); ra.push_back(0.f); } ra.back() += hv[j]; }
  ...
  cudaMalloc(&tmp, tb); cudaMemset(tmp, 0xAB, tb);
  cudaMemcpy(dk, h.data(), 4 * n, cudaMemcpyHostToDevice);
  cudaMemcpy(dv, hv.data(), 4 * n, cudaMemcpyHostToDevice);
  std::vector<K> gu(ru.size()); std::vector<V> ga(ru.size());
  int fails = 0;
  for (int r = 0; r < rounds; ++r)
  {
    cudaMemset(du, 0xCC, 4 * n); cudaMemset(da, 0xCC, 4 * n);
    ...
    if ((size_t) got != ru.size()) { printf("round %d: R got %d ref %zu\n", r, got, ru.size()); ++fails; continue; }
